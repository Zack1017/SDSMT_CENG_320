...
void bigint_trim_void(bigint b);
...
bigint bigint_adc(bigint l, bigint r, chunk carry) {
  ...
  int i, nchunks;
  bigchunk tmpsum;
  // allocate one extra chunk to make sure overflow cannot occur 
...
void cleanup(bigint bint) {

  int temp = bint->size-1; //restore size
  
  //check for number to be valid
  if (temp > 0) 
  {
    ...
    if (bint->blks[temp] == 0) 
    {
      do
        temp--; // removes 0
      while ((temp > 0) && (bint->blks[temp] == 0));
      if (bint->blks[temp] & ((bigchunk)1<<(BITSPERCHUNK-1)))
      {
        temp++; //correction
      }  
    }
    //deals with leading 1 negatice 
    else if (bint->blks[temp] == CHUNKMASK) 
    { 
      do
        temp-=1; // removes 1
      while ((temp>0) && (bint->blks[temp]==CHUNKMASK));
      if (!(bint->blks[temp] & ((bigchunk)1<<(BITSPERCHUNK-1))))
      {
        temp+=1; //correction
      }
  ...
  bint->size = temp; // moves i to correct value to clean up 
  bint->size=bint->size +1; //add one back 
}